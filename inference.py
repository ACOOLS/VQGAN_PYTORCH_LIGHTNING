...
def calculate_ssim(imageA, imageB):
    ...
    score, diff = ssim(imageA, imageB, win_size=7,  full=True)
    print(f"SSIM: {score}")
...
# 	i_x = img.size[0]
# 	#left blacked
# 	j1_left, j2_left = [(0,210) if 'G270' in model else (0,210) if 'Step4' in model else (0,215)][0]
# 	#right blacked
# 	j1_right, j2_right = [(830,1024) if 'G270' in model else (830,1024) if 'Step4' in model else (825,1024)][0]
# 	#top&bottom blacked
# 	i_y = img.size[1]
# 	#top blacked
# 	j1_top, j2_top = [(0,105) if 'G270' in model else (0,105) if 'Step4' in model else (0,100)][0]
# 	#bottom blacked
# 	j1_bottom, j2_bottom = [(850,1024) if 'G270' in model else (850,1024) if 'Step4' in model else (845,1024)][0]
# 	#SN blacked
# 	i1_sn, i2_sn = [(115-5,160+5) if 'G270' in model else (120-5,165+5) if 'Step4' in model else (134-5,169+5)][0]
# 	j1_sn, j2_sn = [(780-5,815+5) if ('G270' in model) or ('Step4' in model) else (750-5,785+5)][0]
# 	#Pad Bottom Left blacked
# 	i1_pbl, i2_pbl = (800-5,850+5)
# 	j1_pbl, j2_pbl = (210-5,260+5)
# 	#Pad Bottom Right blacked
# 	i1_pbr, i2_pbr = (795-5,845+5)
# 	j1_pbr, j2_pbr = (790-5,840+5)
# 	#Number Bottom Left blacked
# 	i1_nbr, i2_nbr = [(700-5,730+5) if ('G270' in model) or ('Step4' in model) else (535-5,565+5)][0]
# 	j1_nbr, j2_nbr = (800-5,830+5)
# 	#SN PCB blacked
# 	i1_snpcb, i2_snpcb = [(435-5,635+5) if ('G270' in model) or ('Step4' in model)  else (0,0)][0]
# 	j1_snpcb, j2_snpcb = [(220-5,240+5) if ('G270' in model) or ('Step4' in model)else (0,0)][0]
# 	#PCB mark top blacked
# 	i1_mtpcb, i2_mtpcb = [(110-5,135+5) if ('G270' in model) or ('Step4' in model) else (100-5,125+5)][0]
# 	j1_mtpcb, j2_mtpcb = (430-5,525+5)
# 	#PCB mark center1 blacked
# 	i1_mc1pcb, i2_mc1pcb = [(210-5,255+5) if ('G270' in model) or ('Step4' in model) else (230-5,315+5)][0]
# 	j1_mc1pcb, j2_mc1pcb = [(530-5,605+5) if ('G270' in model) or ('Step4' in model) else (800-5,825+5)][0]
# 	#PCB mark center2 blacked
# 	i1_mc2pcb, i2_mc2pcb = [(220-5,265+5) if ('G270' in model) or ('Step4' in model) else (317-5,460+5)][0]
# 	j1_mc2pcb, j2_mc2pcb = [(615-5,725+5) if ('G270' in model) or ('Step4' in model) else (800-5,835+5)][0]

# 	crop=np.asarray(img).copy()
# 	for i in range(i_x): # for every y pixels:
# 		for j in range(j1_left, j2_left): #x
# 			crop[i,j] = 0 #left blacked
# 		for j in range(j1_right, j2_right): #x
# 			crop[i,j] = 0 #right blacked
# 	for j in range(i_y): # for every x pixels:
# 		for i in range(j1_top, j2_top): #y
# 			crop[i,j] = 0 #top blacked
# 		for i in range(j1_bottom, j2_bottom): #y
# 			crop[i,j] = 0 #bottom blacked
# 	for i in range(i1_sn,i2_sn): #y 
# 		for j in range(j1_sn,j2_sn): #x
# 			crop[i,j] = 0 #SN blacked
# 	for i in range(i1_pbl, i2_pbl): #y
# 		for j in range(j1_pbl, j2_pbl): #x
# 			crop[i,j] = 0 #pad bottom left blacked
# 	for i in range(i1_pbr, i2_pbr): #y
# 		for j in range(j1_pbr, j2_pbr): #x
# 			crop[i,j] = 0 #pad bottom right blacked
# 	for i in range(i1_nbr, i2_nbr): #y
# 		for j in range(j1_nbr, j2_nbr): #x
# 			crop[i,j] = 0 #number bottom right blacked
# 	for i in range(i1_snpcb, i2_snpcb):  #y 
# 		for j in range(j1_snpcb, j2_snpcb): #x
# 			crop[i,j] = 0 #SN PCB left blacked
# 	for i in range(i1_mtpcb, i2_mtpcb): #y
# 		for j in range(j1_mtpcb, j2_mtpcb): #x
# 			crop[i,j] = 0 #PCB mark top blacked
# 	for i in range(i1_mc1pcb, i2_mc1pcb): #y
# 		for j in range(j1_mc1pcb, j2_mc1pcb): #x
# 			crop[i,j] = 0 #PCB mark center1 blacked
# 	for i in range(i1_mc2pcb, i2_mc2pcb): #y
# 		for j in range(j1_mc2pcb, j2_mc2pcb): #x
# 			crop[i,j] = 0 #PCB mark center2 blacked
...
def reconstruction_pipeline(filepath_picture_IN, filepath_directory_OUT, filepath_config_DL_Model, filepath_DL_Model, patch_size, patch_count, scale_max, scale_step, worst_patch_count,model,size=320):
	...
	x_ori_initial = Image.open(filepath_picture_IN)
	...
	reco = reconstruct_with_vqgan(reco_prep, model1024) # x_ori image PIL [H,W] : 0,255 => reco torch [H,W] : 0,255 converted in a torch [B,H,W]
	x_reco = custom_to_pil(reco[0][0]) # PIL [B,H,W]

	dict_loss = loss(preprocess_vqgan(x_vqgan), model1024)
	...
	reco_homo = reconstruct_with_vqgan(reco_prep_homo, model1024) # x_ori image PIL [H,W] : 0,255 => reco torch [H,W] : 0,255 converted in a torch [B,H,W]
	x_reco_homo = custom_to_pil(reco_homo[0][0]) 
	...
	for weighted in ['weighted']:
		...
		path_parts = filepath_picture_IN.split(os.sep)
...
def main(i_loop,root,current_file,filepath_picture_IN,filepath_directory_OUT,filepath_config_DL_Model,filepath_DL_Model, model):
	...
	metrics, image_to_save, diff_img_2, proportion_white = reconstruction_pipeline(filepath_picture_IN, filepath_directory_OUT, filepath_config_DL_Model, filepath_DL_Model, patch_size, patch_count, scale_max, scale_step, worst_patch_count, model, size=img_size) 
	test_file = os.path.basename(filepath_picture_IN)[:-4]
	...
	# 		print('Image OK')
	#status = 'INFER'
	last_component = os.path.basename(os.path.normpath(root))
	...
	image_to_save.save(filepath_directory_OUT+status+'_'+last_component+'_'+test_file+'_'+str(anomaly_score_ae_total_loss)+'_test'+'.png')
...
i_loop = 0
#for fp in ['NG','NG_new','OK','OK_new','OK_new_complement']:
for test_dir in ['/home/aurelie/datasets/mvtec_anomaly/screw/test']:
    for root, dirs, files in os.walk(test_dir):
        for tf in files:
            if tf.endswith((".jpg", ".png", ".tif",".Tif")):
                i_loop+=1
                print(root+'/'+tf)

                # if 'G270' in tf or 'CCZA' in tf:
                #     model = 'G270'
                #     Mask_HD_model="C:/Users/arb/vqganodip_benchmark/mask_idiff_screw_G270.txt"
                # elif 'ST4' in tf or 'IVZA' in tf or 'IWZC' in tf or 'UUZA' in tf:
                #     model = 'Step4'
                #     Mask_HD_model="C:/Users/arb/vqganodip_benchmark/mask_idiff_screw_Step4.txt"
                # elif 'VA3' in tf or 'VCZA' in tf or 'VDZA' in tf:
                #     model = 'VA3'
                #     Mask_HD_model="C:/Users/arb/vqganodip_benchmark/mask_idiff_screw_VA3.txt"
                if i_loop == 1:
                    current_file = '/home/aurelie/THESE/VQGanoDIP_benchmark_propre/Test/log_result_classique_'+str(round(time()))+'.txt'
				
                output_dir = "/home/aurelie/THESE/VQGanoDIP_benchmark_propre/logs/2024-04-19T13-29-03_custom_vqgan_1CH_screw_classique/"			
                ...
                main(i_loop,
                    root,
                    current_file,
                    filepath_picture_IN=root+'/'+tf, 
                    filepath_directory_OUT=output_dir, 
                    filepath_config_DL_Model="/home/aurelie/THESE/VQGanoDIP_benchmark_propre/logs/2024-04-19T13-29-03_custom_vqgan_1CH_screw_classique/configs/2024-04-19T13-29-03-project.yaml", 
                    filepath_DL_Model="/home/aurelie/THESE/VQGanoDIP_benchmark_propre/logs/2024-04-19T13-29-03_custom_vqgan_1CH_screw_classique/checkpoints/last.ckpt", 
                    model='VQGan')
